...
// ============================================================
// ItemPreview3D — full WebGL preview. Use sparingly (one canvas
// per page). Used inside ItemModal where only one is open at once.
// ============================================================
...
function ShirtPreview({ item, size }: { item: Item; size: number }) {
  ...
  // Community shirt → 3D textured torso
  if (item.imageUrl) {
    return (
      <div
        className="w-full bg-gradient-to-br from-[#EEF0F7] to-[#DDD6F0] cursor-grab active:cursor-grabbing"
        style={{ height: size }}
      >
        <Canvas camera={{ position: [0, 0.1, 2.4], fov: 45 }} dpr={[1, 2]}>
          <ambientLight intensity={0.75} />
          <directionalLight position={[3, 5, 4]} intensity={1.15} />
  ...
  // Built-in shirt → flat-decal overlay
  return (
    <div
      className="w-full bg-gradient-to-br from-[#EEF0F7] to-[#DDD6F0] cursor-grab active:cursor-grabbing"
      style={{ height: size }}
...
export default function ItemPreview3D({ item, size = 160 }: { item: Item; size?: number }) {
